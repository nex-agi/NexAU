...
class PromptBuilder:
    ...
    def _get_base_system_prompt(
        ...
        try:
            ...
            context = self._build_template_context(agent_config, runtime_context)
...
4. Executing the necessary actions and providing clear, helpful responses"""
    ...
    def _build_capabilities_docs(
        ...
        if sub_agent_factories:
            subagent_docs = self._build_subagents_documentation(sub_agent_factories)
    ...
    def _get_tool_execution_instructions(self) -> str:
        ...
        try:
            template = self._load_prompt_template("tool_execution_instructions")
            if template:
                return template
        except Exception as e:
            logger.warning(f"⚠️ Error loading tool execution instructions: {e}")
...
4. Never generate additional content after XML blocks until results are returned"""
    ...
    def _load_prompt_template(self, prompt_name: str) -> str:
        ...
        try:
            ...
            logger.warning(f"⚠️ Error loading prompt template {prompt_name}: {e}")
    ...
    def _build_fallback_prompt(
        ...
        if sub_agent_factories:
            base_prompt += self._build_subagents_documentation_fallback(sub_agent_factories)
    ...
    def _build_tools_documentation_fallback(self, tools: list) -> str:
        ...
        docs = ["\\n\\n## Available Tools"]
        docs.append("You can use tools by including XML blocks in your response:")
        
        for tool in tools:
            docs.append(f"\\n### {tool.name}")
            docs.append(f"{getattr(tool, 'description', 'No description available')}")
            docs.append("Usage:")
