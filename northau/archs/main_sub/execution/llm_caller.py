...
class LLMCaller:
    ...
    def __init__(self, openai_client: Any, llm_config: Any, retry_attempts: int = 5,
    ...
    def call_llm(self, messages: list[dict[str, str]], max_tokens: int, force_stop_reason: str | None = None) -> str:
        ...
        # Call LLM with retry
        response = self._call_with_retry(force_stop_reason=force_stop_reason, **api_params)
    ...
    def _call_with_retry(self, force_stop_reason: str | None = None, **kwargs: Any) -> Any:
        ...
        if force_stop_reason:
            logger.info(f"🛑 LLM call forced to stop due to {force_stop_reason}")
