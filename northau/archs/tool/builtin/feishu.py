...
def extract_post_text(content_obj: dict) -> str:
    ...
    try:
        ...
        has_lang_format = any(key in content_obj for key in ["zh_cn", "en_us", "ja_jp"])
            ...
            for message in message_json["items"]:
                try:
                            ...
                            if msg_type == "text":
                                ...
                                if isinstance(content_obj, dict):
                                    ...
                                    if content_obj.get("text"):
                                        ...
                                        content_text = content_obj.get("message", "")
                                    elif content_obj.get("template"):
                                        # 飞书系统消息通常有template字段，尝试格式化
                                        template = content_obj.get("template", "")
                                        from_user = content_obj.get("from_user", [])
                                        to_chatters = content_obj.get("to_chatters", [])

                                        # 替换常见的模板变量
                                        if from_user:
                                            template = template.replace(
                                                "{from_user}", ", ".join(from_user)
                                            )
                                        if to_chatters:
                                            template = template.replace(
                                                "{to_chatters}", ", ".join(to_chatters)
                                            )
                    ...
                    if sender_type == "user" and sender_id:
                        user_name = user_mapping.get(sender_id, f"用户ID:{sender_id}")
        ...
        try:
            ...
            with open(actual_file_path, "rb") as file_content:
                ...
                if duration is not None:
                    request_body_builder = request_body_builder.duration(duration)
