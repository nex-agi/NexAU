...
def main():
    ...
    print("\n📋 测试 1: 基本命令执行")
    try:
        result = bash_tool("echo 'Hello, World!'", description="Print hello message")
        print(f"✅ 命令执行成功")
    ...
    print("\n📋 测试 2: 错误命令测试")
    try:
        result = bash_tool("ls /nonexistent_directory_12345", description="List non-existent directory")
        if result["status"] != "success":
            print(f"✅ 正确处理了错误命令")
            print(f"🚪 退出码: {result['exit_code']}")
    ...
    print("\n📋 测试 3: 超时测试")
    try:
        result = bash_tool("sleep 1", timeout=500, description="Sleep with short timeout")
    ...
    try:
        ...
        result = bash_tool_instance.execute("pwd", description="Print working directory")
        if result["status"] == "success":
            print(f"✅ 类实现工作正常")
