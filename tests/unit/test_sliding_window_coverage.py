...
# ---------------------------------------------------------------------------
# _generate_summary_direct_fallback
# ---------------------------------------------------------------------------


class TestGenerateSummaryDirectFallback:
    def test_non_openai_raises(self, compaction):
        compaction.summary_llm_config = LLMConfig(
            model="claude-3",
            base_url="https://api.anthropic.com",
            api_key="test-key",
            api_type="anthropic_chat_completion",
        )
        with pytest.raises(RuntimeError, match="only supports openai_chat_completion"):
            compaction._generate_summary_direct_fallback([Message(role=Role.USER, content=[TextBlock(text="Hello")])])

    def test_no_client_raises(self, compaction):
        compaction.summary_llm_config = LLMConfig(
            model="gpt-4o-mini",
            base_url="https://api.openai.com/v1",
            api_key="test-key",
        ...
        compaction._summary_client = None
        with pytest.raises(RuntimeError, match="Summary client is not initialized"):
            compaction._generate_summary_direct_fallback([Message(role=Role.USER, content=[TextBlock(text="Hello")])])

    def test_success_returns_content(self, compaction):
        compaction.summary_llm_config = LLMConfig(
            model="gpt-4o-mini",
            base_url="https://api.openai.com/v1",
            api_key="test-key",
        ...
        compaction._summary_client = mock_client

        result = compaction._generate_summary_direct_fallback([Message(role=Role.USER, content=[TextBlock(text="Hello")])])
        assert result == "Generated summary"

    def test_empty_choices_returns_empty(self, compaction):
        compaction.summary_llm_config = LLMConfig(
            model="gpt-4o-mini",
            base_url="https://api.openai.com/v1",
            api_key="test-key",
