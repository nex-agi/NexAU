...
@asynccontextmanager
async def _create_engine() -> AsyncGenerator[SQLDatabaseEngine]:
    """Create a test SQLDatabaseEngine with automatic cleanup.

    Ensures aiosqlite background threads are stopped by disposing
    the SQLAlchemy engine on exit.
    """
...
class TestSQLDatabaseEngineFromUrl:
    ...
    def test_from_url_sqlite(self):
        ...
        async def run():
            async with _create_engine() as engine:
