...
class Message(BaseModel):
    # Held as ``str`` (not ``UUID``) so legacy / external producers that emit
    # non-UUID ids (e.g. ``"msg-run-{ts}-{rand}-{seq}-history-{idx}"`` from
    # historical chat-history imports) round-trip losslessly. New messages
    # generated by NexAU itself still get a fresh UUID4 string by default,
    # so the format is unchanged for our own writers.
    #
    # All callers historically use ``str(msg.id)`` for dict-keying / filenames,
    # so widening from UUID to str is observation-equivalent for them.
    ...
    def _coerce_id_to_str(cls, v: object) -> object:
        # Accept UUID objects from legacy callers (pre-Phase-1 API) and
        # coerce to string. Pydantic v2's strict str validation rejects
        # UUID instances by default, so without this hook ``Message(id=uuid4())``
        # would raise. Strings (UUID-formatted or arbitrary legacy formats
        # like ``msg-run-{ts}-{rand}-{seq}-history-{idx}``) pass through.
