...
This module contains event-driven streaming architecture for unified LLM provider integration. **Internal use only** — not exposed in user-facing docs.
...
- **Flow**: `aggregate(chunk)` per chunk → `build()` → message. Use `clear()` before reusing.

During `aggregate()`, aggregators optionally emit **unified events** (text deltas, tool calls, etc.) to `on_event`. Use `on_event` for live UI; use `on_event=lambda _: None` when you only need the final message.

**Why this pattern:** (1) Same event interface for all providers. (2) Handles partial JSON, incremental tool-call parsing. (3) UI-agnostic.

## Available aggregators

| Aggregator | Input | Output |
| ---------- | ----- | ------ |
| `OpenAIChatCompletionAggregator` | `ChatCompletionChunk` (Chat Completions API stream) | `ChatCompletion` |
| `OpenAIResponsesAggregator` | `ResponseStreamEvent` (Responses API, e.g. GPT-5 style) | `Response` (reasoning, tool calls, images) |

**Minimal usage** (message only, no-op `on_event`):

```python
from nexau.archs.llm.llm_aggregators import OpenAIChatCompletionAggregator

agg = OpenAIChatCompletionAggregator(on_event=lambda _: None, run_id="run_1")
...
**With streaming events** (see **Event Handler Pattern** below for a full `match event.type` example):
...
**Best practices:** Always `clear()` before reuse; pass `run_id`; use no-op `on_event` when you only need the final message. See **Common Issues** for troubleshooting.
