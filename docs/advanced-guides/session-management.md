...
# Initialize once
engine = SQLDatabaseEngine.from_url("sqlite:///chat.db")
await engine.setup_models()
session_manager = SessionManager(engine=engine)
...
To manage storage, you can query and delete old sessions:

```python
from nexau.archs.session.orm import Repository, SessionModel
...
for session in old_sessions:
    await session_repo.delete(session.user_id, session.session_id)
```
