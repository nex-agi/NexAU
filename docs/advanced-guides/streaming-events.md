...
Events are used in two main places:

1. **LLM Aggregators** — Aggregators emit events as they `aggregate()` vendor chunks. You pass an `on_event` callback when creating the aggregator. See [LLM Aggregators](./llm-aggregators.md).
2. **Agent / Transports** — e.g. `AgentEventsMiddleware` and transport layers emit events during agent runs.
...
Handle events by matching on `type`:
...
## Using events with aggregators

Built-in aggregators accept `on_event` and `run_id`:
...
If you don't need incremental updates, pass a no-op:
...
- [LLM Aggregators](./llm-aggregators.md) — Chunks → vendor message; optional `on_event` during aggregation.
